...
import com.board.exam.config.JwtProvider;
...
public class UserController {
	...
	public ResponseEntity<?> loginchk(@RequestBody User user){
		User principal = userService.login(user.getUsername());
		if(principal == null) {
			principal = new User();
		} else {
			if(encoder.matches(user.getPassword(), principal.getPassword())) {
				principal.setToken(jwtProvider.createToken(principal.getUsername()));
			} else {
				principal = new User();
			}
		}
